       ...
       <div className="flex gap-2 overflow-x-auto no-scrollbar">
          ...
          {BUSINESS_PARTNER_CATEGORIES.map(cat => (
              <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap border transition-all ${
                      activePartnerCategory === cat.label
                      ? 'bg-brand-600 text-white border-brand-600'
                      : 'bg-white text-gray-500 border-gray-200'
                  }`}
              >
             ...
             <div key={partner.id} className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 flex gap-4 group hover:border-brand-200 transition-all relative">
                ...
                <div className="flex-1 min-w-0">
                   <div className="flex flex-col h-full">
                      <div>
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold mb-1 inline-block ${
                             BUSINESS_PARTNER_CATEGORIES.find(c => c.label === partner.category)?.color || 'bg-brand-50 text-brand-600'
                        }`}>
                           {partner.category}
                        </span>
